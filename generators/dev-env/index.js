...
class DevEnv extends yeoman_generator_1.default {
    _copy(dirOrFile, dst = dirOrFile) {
    ...
    writing() {
        ...
        const pkgJson = {
            scripts: {
                ...
                'prettier:lint-fix': 'prettier --write --config .prettierrc.js src/**/*',
                'lint-staged': 'lint-staged',
                'stylelint-check': 'stylelint --print-config .',
                'stylelint:lint': 'stylelint ./src/**/*.{css,less,scss,js,jsx,ts,tsx}; exit 0',
                'stylelint:lint-fix': 'stylelint ./src/**/*.{css,less,scss} --fix',
                'test:clean': 'rimraf ./coverage',
                test: 'cross-env NODE_ENV=test jest --coverage',
                'test:watch': 'cross-env NODE_ENV=test jest --watchAll',
                'build:types': 'tsc --emitDeclarationOnly',
                'build:js': 'babel src --extensions ".ts,.tsx,.js,.jsx" --source-maps inline',
                build: 'npm run build:commonjs && npm run build:umd && npm run build:umd:min && npm run build:es',
                'build:commonjs': 'npm run build:types -- --outDir lib && cross-env BABEL_ENV=commonjs npm run build:js -- --out-dir lib',
                'build:es': 'npm run build:types -- --outDir es && cross-env BABEL_ENV=es npm run build:js -- --out-dir es',
                'build:amd': 'npm run build:types -- --outDir amd && cross-env BABEL_ENV=amd npm run build:js -- --out-dir amd',
                'build:umd': 'npm run build:types -- --outDir umd && cross-env BABEL_ENV=umd npm run build:js -- --out-dir umd',
                'build:umd:min': 'npm run build:types && cross-env BABEL_ENV=commonjs NODE_ENV=production webpack'
            },
            dependencies: {
                '@types/react': '^16.8.13',
